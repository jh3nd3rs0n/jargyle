...
OPTIONS:
  ...
  --enter-chaining-socks-userpassauthmethod-pass
      Enter through an interactive prompt the password to be used to access the other SOCKS server
...
SETTINGS:
  ...
  CHAINING SOCKS5 SETTINGS:
    ...
    chaining.socks5.gssapiauthmethod.mechanismOid=OID
        The object ID for the GSS-API authentication mechanism to the other SOCKS5 server

    chaining.socks5.gssapiauthmethod.necReferenceImpl=true|false
        The boolean value to indicate if the exchange of the GSS-API protection level negotiation must be unprotected should the other SOCKS5 server use the NEC reference implementation

    chaining.socks5.gssapiauthmethod.protectionLevels=SOCKS5_GSSAPIAUTHMETHOD_PROTECTION_LEVELS
        The comma separated list of acceptable protection levels after GSS-API authentication with the other SOCKS5 server (The first is preferred. The remaining are acceptable if the server does not accept the first.)

    chaining.socks5.gssapiauthmethod.serviceName=SERVICE_NAME
        The GSS-API service name for the other SOCKS5 server

    chaining.socks5.gssapiauthmethod.suggestedConf=true|false
        The suggested privacy (i.e. confidentiality) state for GSS-API messages sent after GSS-API authentication with the other SOCKS5 server (applicable if the negotiated protection level is SELECTIVE_INTEG_OR_CONF)

    chaining.socks5.gssapiauthmethod.suggestedInteg=-2147483648-2147483647
        The suggested quality-of-protection (i.e. integrity) value for GSS-API messages sent after GSS-API authentication with the other SOCKS5 server (applicable if the negotiated protection level is SELECTIVE_INTEG_OR_CONF)

    chaining.socks5.methods=SOCKS5_METHODS
        The comma separated list of acceptable authentication methods to the other SOCKS5 server
  ...
  SOCKS SETTINGS:
    ...
    socks.gssapiauthmethod.necReferenceImpl=true|false
    ...
    socks.gssapiauthmethod.protectionLevels=SOCKS_GSSAPIAUTHMETHOD_PROTECTION_LEVELS
    ...
    socks.gssapiauthmethod.suggestedConf=true|false
    ...
    socks.gssapiauthmethod.suggestedInteg=-2147483648-2147483647
    ...
    socks.onBindRequest.listenBindPortRanges=PORT_RANGES
        The comma separated list of binding port ranges for the listen socket if the provided port is zero

    socks.onBindRequest.listenNetInterface=NETWORK_INTERFACE
    ...
    socks.onBindRequest.listenSocketSettings=SOCKET_SETTINGS
        The comma separated list of socket settings for the listen socket

    socks.onBindRequest.relayBufferSize=POSITIVE_INTEGER
        The buffer size in bytes for relaying the data

    socks.onBindRequest.relayIdleTimeout=POSITIVE_INTEGER
        The timeout in milliseconds on relaying no data

    socks.onBindRequest.relayInboundBandwidthLimit=POSITIVE_INTEGER
        The upper limit on bandwidth in bytes per second of receiving inbound data to be relayed

    socks.onBindRequest.relayOutboundBandwidthLimit=POSITIVE_INTEGER
        The upper limit on bandwidth in bytes per second of receiving outbound data to be relayed

    socks.onConnectRequest.prepareTargetFacingSocket=true|false
    ...
    socks.onConnectRequest.relayBufferSize=POSITIVE_INTEGER
        The buffer size in bytes for relaying the data

    socks.onConnectRequest.relayIdleTimeout=POSITIVE_INTEGER
        The timeout in milliseconds on relaying no data

    socks.onConnectRequest.relayInboundBandwidthLimit=POSITIVE_INTEGER
        The upper limit on bandwidth in bytes per second of receiving inbound data to be relayed

    socks.onConnectRequest.relayOutboundBandwidthLimit=POSITIVE_INTEGER
        The upper limit on bandwidth in bytes per second of receiving outbound data to be relayed

    socks.onConnectRequest.targetFacingBindHost=HOST
        The binding host name or address for the target-facing socket

    socks.onConnectRequest.targetFacingBindHostAddressTypes=HOST_ADDRESS_TYPES
        The comma separated list of acceptable binding host address types for the target-facing socket

    socks.onConnectRequest.targetFacingBindPortRanges=PORT_RANGES
        The comma separated list of binding port ranges for the target-facing socket

    socks.onConnectRequest.targetFacingConnectTimeout=POSITIVE_INTEGER
        The timeout in milliseconds on waiting for the target-facing socket to connect (default value is 60000)

    socks.onConnectRequest.targetFacingNetInterface=NETWORK_INTERFACE
        The network interface that provides a binding host address for the target-facing socket

    socks.onConnectRequest.targetFacingSocketSettings=SOCKET_SETTINGS
        The comma separated list of socket settings for the target-facing socket

    socks.onRequest.externalFacingBindHost=HOST
        The binding host name or address for all external-facing sockets

    socks.onRequest.externalFacingBindHostAddressTypes=HOST_ADDRESS_TYPES
        The comma separated list of acceptable binding host address types for all external-facing sockets

    socks.onRequest.externalFacingBindTcpPortRanges=PORT_RANGES
        The comma separated list of binding port ranges for all external-facing TCP sockets

    socks.onRequest.externalFacingBindUdpPortRanges=PORT_RANGES
        The comma separated list of binding port ranges for all external-facing UDP sockets

    socks.onRequest.externalFacingNetInterface=NETWORK_INTERFACE
        The network interface that provides a binding host address for all external-facing sockets

    socks.onRequest.externalFacingSocketSettings=SOCKET_SETTINGS
        The comma separated list of socket settings for all external-facing sockets

    socks.onRequest.internalFacingBindHost=HOST
        The binding host name or address for all internal-facing sockets

    socks.onRequest.internalFacingBindHostAddressTypes=HOST_ADDRESS_TYPES
        The comma separated list of acceptable binding host address types for all internal-facing sockets

    socks.onRequest.internalFacingBindUdpPortRanges=PORT_RANGES
        The comma separated list of binding port ranges for all internal-facing UDP sockets

    socks.onRequest.internalFacingNetInterface=NETWORK_INTERFACE
        The network interface that provides a binding host address for all internal-facing sockets

    socks.onRequest.internalFacingSocketSettings=SOCKET_SETTINGS
        The comma separated list of socket settings for all internal-facing sockets

    socks.onRequest.relayBufferSize=POSITIVE_INTEGER
        The buffer size in bytes for relaying the data (default value is 1024)

    socks.onRequest.relayIdleTimeout=POSITIVE_INTEGER
        The timeout in milliseconds on relaying no data (default value is 60000)

    socks.onRequest.relayInboundBandwidthLimit=POSITIVE_INTEGER
        The upper limit on bandwidth in bytes per second of receiving inbound data to be relayed

    socks.onRequest.relayOutboundBandwidthLimit=POSITIVE_INTEGER
        The upper limit on bandwidth in bytes per second of receiving outbound data to be relayed

    socks.onUdpAssociateRequest.clientFacingBindHost=HOST
        The binding host name or address for the client-facing UDP socket

    socks.onUdpAssociateRequest.clientFacingBindHostAddressTypes=HOST_ADDRESS_TYPES
        The comma separated list of acceptable binding host address types for the client-facing UDP socket

    socks.onUdpAssociateRequest.clientFacingBindPortRanges=PORT_RANGES
        The comma separated list of binding port ranges for the client-facing UDP socket

    socks.onUdpAssociateRequest.clientFacingNetInterface=NETWORK_INTERFACE
        The network interface that provides a binding host address for the client-facing UDP socket

    socks.onUdpAssociateRequest.clientFacingSocketSettings=SOCKET_SETTINGS
        The comma separated list of socket settings for the client-facing UDP socket

    socks.onUdpAssociateRequest.peerFacingBindHost=HOST
        The binding host name or address for the peer-facing UDP socket

    socks.onUdpAssociateRequest.peerFacingBindHostAddressTypes=HOST_ADDRESS_TYPES
        The comma separated list of acceptable binding host address types for the peer-facing UDP socket

    socks.onUdpAssociateRequest.peerFacingBindPortRanges=PORT_RANGES
        The comma separated list of binding port ranges for the peer-facing UDP socket

    socks.onUdpAssociateRequest.peerFacingNetInterface=NETWORK_INTERFACE
        The network interface that provides a binding host address for the peer-facing UDP socket

    socks.onUdpAssociateRequest.peerFacingSocketSettings=SOCKET_SETTINGS
        The comma separated list of socket settings for the peer-facing UDP socket

    socks.onUdpAssociateRequest.relayBufferSize=POSITIVE_INTEGER
        The buffer size in bytes for relaying the data

    socks.onUdpAssociateRequest.relayIdleTimeout=POSITIVE_INTEGER
        The timeout in milliseconds on relaying no data

    socks.onUdpAssociateRequest.relayInboundBandwidthLimit=POSITIVE_INTEGER
        The upper limit on bandwidth in bytes per second of receiving inbound data to be relayed

    socks.onUdpAssociateRequest.relayOutboundBandwidthLimit=POSITIVE_INTEGER
        The upper limit on bandwidth in bytes per second of receiving outbound data to be relayed

    socks.userpassauthmethod.userRepository=SOCKS_USERPASSAUTHMETHOD_USER_REPOSITORY
        The user repository used for username password authentication (default value is StringSourceUserRepository:)

  SOCKS5 SETTINGS:

    socks5.gssapiauthmethod.necReferenceImpl=true|false
        The boolean value to indicate if the exchange of the GSS-API protection level negotiation must be unprotected according to the NEC reference implementation

    socks5.gssapiauthmethod.protectionLevels=SOCKS5_GSSAPIAUTHMETHOD_PROTECTION_LEVELS
        The comma separated list of acceptable protection levels after GSS-API authentication (The first is preferred if the client does not provide a protection level that is acceptable.)

    socks5.gssapiauthmethod.suggestedConf=true|false
        The suggested privacy (i.e. confidentiality) state for GSS-API messages sent after GSS-API authentication (applicable if the negotiated protection level is SELECTIVE_INTEG_OR_CONF)

    socks5.gssapiauthmethod.suggestedInteg=-2147483648-2147483647
        The suggested quality-of-protection (i.e. integrity) value for GSS-API messages sent after GSS-API authentication (applicable if the negotiated protection level is SELECTIVE_INTEG_OR_CONF)

    socks5.methods=SOCKS5_METHODS
        The comma separated list of acceptable authentication methods in order of preference
    ...
    socks5.onConnectRequest.prepareTargetFacingSocket=true|false
        The boolean value to indicate if the target-facing socket is to be prepared before connecting (involves applying the specified socket settings, resolving the target host name, and setting the specified timeout on waiting to connect)
    ...
    socks5.onConnectRequest.targetFacingConnectTimeout=POSITIVE_INTEGER
        The timeout in milliseconds on waiting for the target-facing socket to connect
    ...
    socks5.userpassauthmethod.userRepository=SOCKS5_USERPASSAUTHMETHOD_USER_REPOSITORY
...
VALUE TYPES:
  ...
  RULE_ACTION: NAME=VALUE
    ...
    SOCKS RULE ACTIONS:
      ...
      socks.onBindRequest.inboundSocketSetting=SOCKET_SETTING
      ...
      socks.onBindRequest.listenBindHostAddressType=HOST_ADDRESS_TYPE
      ...
      socks.onBindRequest.listenBindPortRange=PORT_RANGE
      ...
      socks.onBindRequest.listenSocketSetting=SOCKET_SETTING
      ...
      socks.onConnectRequest.targetFacingBindHostAddressType=HOST_ADDRESS_TYPE
      ...
      socks.onConnectRequest.targetFacingBindPortRange=PORT_RANGE
      ...
      socks.onConnectRequest.targetFacingSocketSetting=SOCKET_SETTING
      ...
      socks.onRequest.externalFacingBindHostAddressType=HOST_ADDRESS_TYPE
          Specifies an acceptable binding host address type for all external-facing sockets (can be specified multiple times with each rule action specifying another host address type)

      socks.onRequest.externalFacingBindTcpPortRange=PORT_RANGE
          Specifies a binding port range for all external-facing TCP sockets (can be specified multiple times with each rule action specifying another port range)

      socks.onRequest.externalFacingBindUdpPortRange=PORT_RANGE
          Specifies a binding port range for all external-facing UDP sockets (can be specified multiple times with each rule action specifying another port range)

      socks.onRequest.externalFacingNetInterface=NETWORK_INTERFACE
          Specifies the network interface that provides a binding host address for all external-facing sockets

      socks.onRequest.externalFacingSocketSetting=SOCKET_SETTING
          Specifies a socket setting for all external-facing sockets (can be specified multiple times with each rule action specifying another socket setting)

      socks.onRequest.internalFacingBindHost=HOST
          Specifies the binding host name or address for all internal-facing sockets

      socks.onRequest.internalFacingBindHostAddressType=HOST_ADDRESS_TYPE
          Specifies an acceptable binding host address type for all internal-facing sockets (can be specified multiple times with each rule action specifying another host address type)

      socks.onRequest.internalFacingBindUdpPortRange=PORT_RANGE
          Specifies a binding port range for all internal-facing UDP sockets (can be specified multiple times with each rule action specifying another port range)

      socks.onRequest.internalFacingNetInterface=NETWORK_INTERFACE
          Specifies the network interface that provides a binding host address for all internal-facing sockets

      socks.onRequest.internalFacingSocketSetting=SOCKET_SETTING
          Specifies a socket setting for all internal-facing sockets (can be specified multiple times with each rule action specifying another socket setting)
      ...
      socks.onUdpAssociateRequest.clientFacingBindHostAddressType=HOST_ADDRESS_TYPE
      ...
      socks.onUdpAssociateRequest.clientFacingBindPortRange=PORT_RANGE
      ...
      socks.onUdpAssociateRequest.clientFacingSocketSetting=SOCKET_SETTING
      ...
      socks.onUdpAssociateRequest.peerFacingBindHostAddressType=HOST_ADDRESS_TYPE
      ...
      socks.onUdpAssociateRequest.peerFacingBindPortRange=PORT_RANGE
      ...
      socks.onUdpAssociateRequest.peerFacingSocketSetting=SOCKET_SETTING
      ...
      socks.request.desiredDestinationAddressRedirect=ADDRESS
          Specifies the desired destination address redirect for the request.

      socks.request.desiredDestinationPortRedirect=PORT
          Specifies the desired destination port redirect for the request

      socks.request.desiredDestinationRedirectLogAction=LOG_ACTION
  ...
  RULE_CONDITION: NAME=VALUE
    ...
    SOCKS RULE CONDITIONS:
      ...
      socks.method=SOCKS_METHOD
          Specifies the negotiated method.

      socks.reply.serverBoundAddress=ADDRESS_RANGE
          Specifies the server bound address of the reply

      socks.reply.serverBoundPort=PORT_RANGE
      ...
      socks.request.desiredDestinationPort=PORT_RANGE
          Specifies the desired destination port of the request

      socks.secondReply.serverBoundAddress=ADDRESS_RANGE
          Specifies the server bound address of the second reply (for the BIND request)

      socks.secondReply.serverBoundPort=PORT_RANGE
          Specifies the server bound port of the second reply (for the BIND request)

      socks.udp.inbound.desiredDestinationAddress=ADDRESS_RANGE
          Specifies the UDP inbound desired destination address

      socks.udp.inbound.desiredDestinationPort=PORT_RANGE
          Specifies the UDP inbound desired destination port

      socks.udp.inbound.sourceAddress=ADDRESS_RANGE
          Specifies the UDP inbound source address

      socks.udp.inbound.sourcePort=PORT_RANGE
          Specifies the UDP inbound source port

      socks.udp.outbound.desiredDestinationAddress=ADDRESS_RANGE
          Specifies the UDP outbound desired destination address

      socks.udp.outbound.desiredDestinationPort=PORT_RANGE
          Specifies the UDP outbound desired destination port

      socks.udp.outbound.sourceAddress=ADDRESS_RANGE
          Specifies the UDP outbound source address

      socks.udp.outbound.sourcePort=PORT_RANGE
  ...
  SOCKS5_GSSAPIAUTHMETHOD_PROTECTION_LEVEL: NONE|REQUIRED_INTEG|REQUIRED_INTEG_AND_CONF|SELECTIVE_INTEG_OR_CONF
  ...
  SOCKS5_GSSAPIAUTHMETHOD_PROTECTION_LEVELS: SOCKS5_GSSAPIAUTHMETHOD_PROTECTION_LEVEL1[,SOCKS5_GSSAPIAUTHMETHOD_PROTECTION_LEVEL2[...]]
  ...
  SOCKS_SERVER_URI: SOCKS_SERVER_URI_SCHEME://[USER_INFO@]HOST[:PORT]

  SOCKS_SERVER_URI_SCHEME: socks5
