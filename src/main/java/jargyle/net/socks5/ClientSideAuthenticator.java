...
import jargyle.net.socks5.gssapiauth.GssapiProtectionLevel;
import jargyle.net.socks5.gssapiauth.GssapiProtectionLevels;
import jargyle.net.socks5.gssapiauth.Message;
import jargyle.net.socks5.gssapiauth.MessageType;
import jargyle.net.socks5.gssapiauth.ProtectionLevel;
import jargyle.net.socks5.userpassauth.UsernamePasswordRequest;
import jargyle.net.socks5.userpassauth.UsernamePasswordResponse;
import jargyle.security.EncryptedPassword;

enum ClientSideAuthenticator {
	...
	public static ClientSideAuthenticator valueOf(final Method meth) {
		for (ClientSideAuthenticator value : ClientSideAuthenticator.values()) {
			if (value.methodValue().equals(meth)) {
				return value;
			}
		}
		StringBuilder sb = new StringBuilder();
		List<ClientSideAuthenticator> list = Arrays.asList(ClientSideAuthenticator.values());
		for (Iterator<ClientSideAuthenticator> iterator = list.iterator();
				iterator.hasNext();) {
			ClientSideAuthenticator value = iterator.next();
	...
	private ClientSideAuthenticator(final Method methValue) {
