...
public final class SocksClientFactory {
	...
	private static List<Property> getSocks5ClientProperties(
		...
		if (settings.containsNondefaultValue(
				SettingSpec.CHAIN_AGENT_SOCKS5_AUTH_METHODS)) {
		...
		if (settings.containsNondefaultValue(
				SettingSpec.CHAIN_AGENT_SOCKS5_GSSAPI_MECHANISM_OID)) {
		...
		if (settings.containsNondefaultValue(
				SettingSpec.CHAIN_AGENT_SOCKS5_GSSAPI_NEC_REFERENCE_IMPL)) {
		...
		if (settings.containsNondefaultValue(
				SettingSpec.CHAIN_AGENT_SOCKS5_GSSAPI_PROTECTION_LEVELS)) {
		...
		if (settings.containsNondefaultValue(
				SettingSpec.CHAIN_AGENT_SOCKS5_GSSAPI_SERVICE_NAME)) {
		...
		if (settings.containsNondefaultValue(
				SettingSpec.CHAIN_AGENT_SOCKS5_USERNAME_PASSWORD)) {
	...
	private static List<Property> getSocksClientProperties(
		...
		List<Property> properties = new ArrayList<Property>();
		if (settings.containsNondefaultValue(
				SettingSpec.CHAIN_AGENT_BIND_HOST)) {
			Host bindHost = settings.getLastValue(
					SettingSpec.CHAIN_AGENT_BIND_HOST, Host.class);
			properties.add(PropertySpec.BIND_HOST.newProperty(bindHost));
		}
		if (settings.containsNondefaultValue(
				SettingSpec.CHAIN_AGENT_CONNECT_TIMEOUT)) {
		...
		if (settings.containsNondefaultValue(
				SettingSpec.CHAIN_AGENT_SOCKET_SETTINGS)) {
		...
		if (settings.containsNondefaultValue(
				SettingSpec.CHAIN_AGENT_SSL_ENABLED)) {
			Boolean sslEnabled = settings.getLastValue(
					SettingSpec.CHAIN_AGENT_SSL_ENABLED, Boolean.class);
			properties.add(PropertySpec.SSL_ENABLED.newProperty(sslEnabled));
		}
		if (settings.containsNondefaultValue(
				SettingSpec.CHAIN_AGENT_SSL_ENABLED_CIPHER_SUITES)) {
		...
		if (settings.containsNondefaultValue(
				SettingSpec.CHAIN_AGENT_SSL_ENABLED_PROTOCOLS)) {
		...
		if (settings.containsNondefaultValue(
				SettingSpec.CHAIN_AGENT_SSL_KEY_STORE_FILE)) {
			File sslKeyStoreFile = settings.getLastValue(
					SettingSpec.CHAIN_AGENT_SSL_KEY_STORE_FILE, File.class);
			properties.add(PropertySpec.SSL_KEY_STORE_FILE.newProperty(
					sslKeyStoreFile));
		}
		if (settings.containsNondefaultValue(
				SettingSpec.CHAIN_AGENT_SSL_KEY_STORE_PASSWORD)) {
		...
		if (settings.containsNondefaultValue(
				SettingSpec.CHAIN_AGENT_SSL_KEY_STORE_TYPE)) {
		...
		if (settings.containsNondefaultValue(
				SettingSpec.CHAIN_AGENT_SSL_PROTOCOL)) {
			String sslProtocol = settings.getLastValue(
					SettingSpec.CHAIN_AGENT_SSL_PROTOCOL, String.class);
			properties.add(PropertySpec.SSL_PROTOCOL.newProperty(sslProtocol));
		}
		if (settings.containsNondefaultValue(
				SettingSpec.CHAIN_AGENT_SSL_TRUST_STORE_FILE)) {
		...
		if (settings.containsNondefaultValue(
				SettingSpec.CHAIN_AGENT_SSL_TRUST_STORE_PASSWORD)) {
		...
		if (settings.containsNondefaultValue(
				SettingSpec.CHAIN_AGENT_SSL_TRUST_STORE_TYPE)) {
	...
	public static SocksClient newSocksClient(
		...
		SocksServerUri socksServerUri = settings.getLastValue(
				SettingSpec.CHAIN_AGENT_SOCKS_SERVER_URI, SocksServerUri.class);
